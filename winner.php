...
d17/x3f8m6l++P5/85njqecf6NH/s8/0/wHeyCdEnMVhkQAAAABJRU5ErkJggg==" align="right" style="padding-top: 40px;">
					<p>
					<b>  יקרים  <?php echo SERVER_NAME; ?>  שחקני, </b>
					<br /><br />
					כל הדברים טובים חייבים לבוא לסיומו, וכך גם חובת גיל זה. ברגע שסולומון קבל טבעת, שעליה היה כתוב הודעה שיכול לקחת את כל
                                                                                את ההנאות או כאבים של העולם, כי המסר היה תרגום גס "גם זה יעבור". זה גם השמחה והצער שלנו להודיע ??לכל השחקנים ש
					זה גם עתה עבר! אנו מקווים שאתה נהנה את הזמן שלך איתנו כמו שאנחנו נהנים לשרת אותך ומודים לך על שנשארתם עד הסוף!<br /><br />

					התוצאות: היום חלף זמן רב מאז לתוך הלילה, אך העובדים ב <?php echo "<a href=\"karte.php?d=$vref&c=".$generator->getMapCheck($vref)."\">$winningvillagename</a>"; ?>,
					עמל על הפלא עולם חורפי הערב, כל להיזהר מהצבאות אינספור צועדים להרוס את העבודה שלהם, בידיעה שהם רצו נגד זמן והגדול ביותר
                                                                                איום שמעולם לא עמד בפני האנשים החופשיים. המאבקים הבלתי הנלאים שלהם  <strike><b>זמן</b></strike> לא <strike><b>תאריך</b></strike> after a
					עובדת אלמונית הניחה את אבן הכוח במה ידוע לנצח כמו יצירה הגדולה והמפוארת ביותר בכל ההיסטוריה מאז נפילת הנטארים<br /><br />

					יחד עם הברית "<?php echo "<a href=\"allianz.php?aid=$allianceid\">$winningalliancetag</a>"; ?>", "<?php echo "<a href=\"spieler.php?uid=$owner\">$username</a>"; ?>"היה הראשון שסיים את פלא העולם, תוך שימוש במיליוני משאבים תוך הגנה עליו גם עם עזרת אלף מגינים אמיצים. לכן זה <b><?php echo "<a href=\"spieler.php?uid=$owner\">$username</a>"; ?></b>
					מי שמקבל זוכת התואר "של עידן זה"!<br /><br />


					"<a href="spieler.php?uid=<?php echo $datas[0]['userid']; ?>" title="Total Villages: <?php echo $datas[0]['totalvillages']; echo "\n";?>Total Population: <?php echo $datas[0]['totalpop']; ?>"><?php echo $datas[0]['username']; ?></a>" היה שליט על האימפריה הגדולה ביותר האישית, ובעקבותיו"<a href="spieler.php?uid=<?php echo $datas[1]['userid']; ?>" title="Total Villages: <?php echo $datas[1]['totalvillages']; echo "\n";?>Total Population: <?php echo $datas[1]['totalpop']; ?>"><?php echo $datas[1]['username']; ?></a>" and "<a href="spieler.php?uid=<?php echo $datas[2]['userid']; ?>" title="Total Villages: <?php echo $datas[2]['totalvillages']; echo "\n";?>Total Population: <?php echo $datas[2]['totalpop']; ?>"><?php echo $datas[2]['username']; ?></a>".<br />
					"<a href="spieler.php?uid=<?php echo $attacker[0]['userid']; ?>" title="Total Villages: <?php echo $attacker[0]['totalvillages']; echo "\n"; ?>Attack Points: <?php echo $attacker[0]['apall']; ?>"><?php echo $attacker[0]['username']; ?></a>" הרג יותר מכל דבר אחר, והיה המפקד החזק ביותר, המפחיד ביותר.<br />
					"<a href="spieler.php?uid=<?php echo $defender[0]['userid']; ?>" title="Total Villages: <?php echo $defender[0]['totalvillages']; echo "\n"; ?>Defence Points: <?php echo $defender[0]['dpall'];?>"><?php echo $defender[0]['username']; ?></a>" היה מגן המהולל ביותר, שחיטת אויבים בשערי הכפר, מכתימה את האדמות סביב הכפרים האלה בדמם.
					<br /><br />
					<p>ברכות לכולם.</p>
					<br /><br />
					בברכה,<br />
					<?php echo SERVER_NAME; ?> צוות<br /><br /><br /><br />
					<small><i></i></small></p>

					<br /><br />
					<center><a href="dorf1.php">&raquo; המשך לכפר</a></center>
				</div>
			...
			<div id="stime">
				<div id="ltime">
					<div id="ltimeWrap">
						מחושב ב <b><?php echo round(($generator->pageLoadTimeEnd()-$start)*1000);?></b> ms
